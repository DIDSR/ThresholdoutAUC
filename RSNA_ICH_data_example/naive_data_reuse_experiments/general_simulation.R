...
# The parameter `iter_i` denotes the i'th repetition of the experiments, whereby in each
# repetition a different subset of the RSNA ICH dataset is used for model training and testing.
fit_models_rsna_ich <- function(classifier = "glmnet", iter_i, n_train,
  ...
  for (round_ind in 1:n_adapt_rounds) {
    ...
    features_to_keep <- union(features_to_keep,
                              selected_features[[best_model_ind]])
